...
- 如果
  ...
  $$
  A(\mathbf{z}^* , \mathbf{z}^{(\tau)})= min{\left( 1,\frac{\tilde p(\mathbf{z}^*) q(\mathbf{z}^{(\tau)}|\mathbf{z}^* )}{\tilde p(\mathbf{z}^{(\tau)}) q(\mathbf{z}^*| \mathbf{z}^{(\tau)})}\right) }>u
  $$
